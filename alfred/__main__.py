...
custom_error_handling(Alfred, logger)
try:
    with open(os.path.join('data', 'serverdict'), 'rb') as fd:
        Alfred.guild_dict = pickle.load(fd)
    ...
    try:
        ...
        with open(os.path.join('data', 'serverdict'), 'wb') as fd:
            pickle.dump(Alfred.guild_dict, fd, (-1))
        logger.info('Serverdict Created')

guild_dict = Alfred.guild_dict
...
# Assumes that config files will always live
...
def load_config():
    ...
    language = gettext.translation(
        'alfred', localedir='locale', languages=[config['bot-language']]
    )
    language.install()
    ...
    # Set spelling dictionary for our list of Pokemon
    pkmn_match.set_list(pkmn_info['pokemon_list'])
    return (pokemon_path_source, raid_path_source)

pkmn_path, raid_path = load_config()

Alfred.pkmn_info = pkmn_info
Alfred.raid_info = raid_info
Alfred.type_chart = type_chart
Alfred.type_list = type_list

Alfred.config = config
Alfred.pkmn_info_path = pkmn_path
Alfred.raid_json_path = raid_path

default_exts = ['datahandler', 'tutorial', 'silph', 'utilities', 'pokemon', 'trade']

for ext in default_exts:
    try:
        Alfred.load_extension(f"alfred.exts.{ext}")
    except Exception as e:
        print(f'**Error when loading extention {ext}:**\n{type(e).__name__}: {e}')
    else:
        if 'debug' in sys.argv[1:]:
            print(f'Loaded {ext} extension.')

@Alfred.command(name='load')
@checks.is_owner()
async def _load(ctx, *extensions):
    for ext in extensions:
        try:
            ctx.bot.unload_extension(f"alfred.exts.{ext}")
            ctx.bot.load_extension(f"alfred.exts.{ext}")
        except Exception as e:
...
@Alfred.command(name='unload')
@checks.is_owner()
async def _unload(ctx, *extensions):
    exts = [e for e in extensions if f"exts.{e}" in Alfred.extensions]
    for ext in exts:
...
# Given a User, check that it is Alfred's master
...
async def ask(message, destination, user_list=None, *, react_list=['✅', '❎']):
    ...
    try:
        reaction, user = await Alfred.wait_for('reaction_add', check=check, timeout=60)
...
def get_gyms(guild_id):
    gym_matching_cog = Alfred.cogs.get('GymMatching')
    ...
    if type == "exraid":
        ...
        if guild_dict[guild.id]['configure_dict']['invite']['enabled']:
            ...
            if guild_dict[guild.id]['configure_dict']['exraid']['permissions'] == "everyone":
                ...
                raid_channel_overwrite_list.append(everyone_overwrite)
        alfred_overwrite = (Alfred.user, discord.PermissionOverwrite(send_messages=True, read_messages=True, manage_roles=True))
        raid_channel_overwrite_list.append(alfred_overwrite)
...
async def expiry_check(channel):
    ...
    guild = channel.guild
    global active_raids
    channel = Alfred.get_channel(channel.id)
...
async def expire_channel(channel):
    ...
    channel_exists = Alfred.get_channel(channel.id)
    channel = channel_exists
    if (channel_exists == None) and (not Alfred.is_closed()):
        try:
        ...
        try:
            if (guild_dict[channel.guild.id]['raidchannel_dict'][channel.id]['active'] == False) and (not Alfred.is_closed()):
                if dupechannel:
                    try:
                        report_channel = Alfred.get_channel(
                ...
                if channel_exists == None:
                    ...
                    if new_name not in channel.name:
                        ...
                        await channel.send(_('-----------------------------------------------\n**The channel has been archived and removed from view for everybody but Alfred and those with Manage Channel permissions. Any messages that were deleted after the channel was marked for archival will be posted below. You will need to delete this channel manually.**\n-----------------------------------------------'))
                        while logs:
                            earliest = min(logs)
...
Alfred.expire_channel = expire_channel

async def channel_cleanup(loop=True):
    while (not Alfred.is_closed()):
        global active_raids
        ...
        for guildid in guilddict_chtemp.keys():
            guild = Alfred.get_guild(guildid)
            ...
            for channelid in guilddict_chtemp[guildid]['raidchannel_dict']:
                ...
                # otherwise, if alfred can still see the channel in discord
                else:
                    logger.info(
            ...
            for report_dict in report_dict_dict:
                for reportid in report_dict_dict[report_dict].keys():
                    if report_dict_dict[report_dict][reportid].get('exp', 0) <= time.time():
                        report_channel = Alfred.get_channel(report_dict_dict[report_dict][reportid].get('reportchannel'))
                        if report_channel:
...
@Alfred.event
async def on_ready():
    Alfred.owner = discord.utils.get(
        Alfred.get_all_members(), id=config['master'])
    await _print(Alfred.owner, _('Starting up...'))
    Alfred.uptime = datetime.datetime.now()
    msg_success = 0
    msg_fail = 0
    guilds = len(Alfred.guilds)
    users = 0
    for guild in Alfred.guilds:
    ...
    await _print(Alfred.owner, _("Ready."))
    await maint_start()
...
async def on_guild_join(guild):
    ...
    await owner.send(_("I'm Alfred, a Discord helper bot for Pokemon Go communities, and someone has invited me to your server! Type **!help** to see a list of things I can do, and type **!configure** in any channel of your server to begin!"))
...
async def on_message(message):
    if message.guild != None:
        if message.author == message.guild.me:
            return
        mguild = message.channel.id
        if message.channel.id == 511733961597255681:
            for attachment in message.attachments:
                url = attachment.url
                filename = 'raidscreenshot4.png'
                ...
                if p.search(ptime):
                    posttime = p.search(ptime).groups()
                else:
                ...
                print(pytesseract.image_to_string(Image.open(timeimg)))
                print('Done.')
        ...
        if raid_status:
            ...
            if guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['active']:
                ...
                if "/maps" in message.content and "http" in message.content:
                    ...
                    report_channel = Alfred.get_channel(guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['reportcity'])
    ...
    if (not message.author.bot):
        await Alfred.process_commands(message)
...
async def on_raw_reaction_add(payload):
    channel = Alfred.get_channel(payload.channel_id)
    try:
    ...
    try:
        ...
        return
    guild = message.guild
    if channel.id in guild_dict[guild.id]['raidchannel_dict'] and user.id != Alfred.user.id:
        if message.id == guild_dict[guild.id]['raidchannel_dict'][channel.id].get('ctrsmessage',None):
            ...
            if str(payload.emoji) == '\u2754':
                ...
                prefix = prefix or Alfred.config['default_prefix']
                avatar = Alfred.user.avatar_url
    ...
    try:
        ...
        wildreport_dict = []
    if message.id in wildreport_dict and user.id != Alfred.user.id:
...
Admin Commands
"""
@Alfred.command(hidden=True, name="eval")
@checks.is_owner()
...
async def save(ctx):
    ...
    try:
        ...
        await _print(Alfred.owner, _('Error occured while trying to save!'))
        await _print(Alfred.owner, err)
...
async def restart(ctx):
    ...
    Usage: !restart.
    Calls the save function and restarts Alfred."""
    try:
        await _save()
    except Exception as err:
        await _print(Alfred.owner, _('Error occured while trying to save!'))
...
async def exit(ctx):
    ...
    Alfred._shutdown_mode = 0
    await Alfred.logout()

@Alfred.group(name='set', case_insensitive=True)
async def _set(ctx):
    """Changes a setting."""
...
async def regional(ctx, regional):
    ...
    if regional.isdigit():
        ...
        if regional == "reset" and checks.is_dev_or_owner(ctx):
            ...
            regional = None
            _set_regional(Alfred, ctx.guild, regional)
...
async def timezone(ctx,*, timezone: str = ''):
    ...
    if (not ((- 12) <= timezone <= 14)):
        await ctx.channel.send(_("I couldn't convert your answer to an appropriate timezone! Please double check what you sent me and resend a number from **-12** to **12**."))
        return
    _set_timezone(Alfred, ctx.guild, timezone)
...
async def prefix(ctx, prefix=None):
    ...
    if prefix == 'clear':
        prefix = None
    prefix = prefix.strip()
    _set_prefix(Alfred, ctx.guild, prefix)
    if prefix != None:
        await ctx.channel.send(_('Prefix has been set to: `{}`').format(prefix))
    else:
        default_prefix = Alfred.config['default_prefix']
...
@Alfred.group(name='get', case_insensitive=True)
@commands.has_permissions(manage_guild=True)
async def _get(ctx):
    """Get a setting value"""
    if ctx.invoked_subcommand == None:
...
async def outputlog(ctx):
    """Get current Alfred log.

    Usage: !outputlog
    Output is a link to hastebin."""
    with open(os.path.join('logs', 'alfred.log'), 'r', encoding='latin-1', errors='replace') as logfile:
        logdata = logfile.read()
    await ctx.channel.send(hastebin.post(logdata))

@Alfred.command(aliases=['say'])
@commands.has_permissions(manage_guild=True)
async def announce(ctx, *, announce=None):
    """Repeats your message in an embed from Alfred.

    Usage: !announce [announcement]
    If the announcement isn't added at the same time as the command, Alfred will wait 3 minutes for a followup message containing the announcement."""
    message = ctx.message
    channel = message.channel
    guild = message.guild
    author = message.author
    if announce == None:
        ...
        announcemsg = await Alfred.wait_for('message', timeout=180, check=(lambda reply: reply.author == message.author))
    ...
    if ctx.invoked_with == "announce":
        ...
        if Alfred.user.avatar_url:
            embeddraft.set_author(name=title, icon_url=Alfred.user.avatar_url)
        else:
    ...
    if not timeout:
        ...
        if res.emoji == '❎':
            ...
            channelmsg = await Alfred.wait_for('message', timeout=60, check=(lambda reply: reply.author == message.author))
            if channelmsg.content.isdigit():
                sendchannel = Alfred.get_channel(int(channelmsg.content))
            elif channelmsg.raw_channel_mentions:
                sendchannel = Alfred.get_channel(channelmsg.raw_channel_mentions[0])
            else:
...
@Alfred.group(case_insensitive=True, invoke_without_command=True)
@commands.has_permissions(manage_guild=True)
async def configure(ctx, *, configlist: str=""):
    """Alfred Configuration

    Usage: !configure [list]
    Alfred will DM you instructions on how to configure Alfred for your server.
...
async def _configure(ctx, configlist):
    ...
    if not firstconfig and not configreplylist:
        ...
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=configmessage).set_author(name=_('Alfred Configuration - {guild}').format(guild=guild.name), icon_url=Alfred.user.avatar_url))
        while True:
            def check(m):
                return m.guild == None and m.author == owner
            configreply = await Alfred.wait_for('message', check=check)
    ...
    try:
        ...
        if ctx:
            ...
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_("Alright! Your settings have been saved and I'm ready to go! If you need to change any of these settings, just type **!configure** in your server again.")).set_author(name=_('Configuration Complete'), icon_url=Alfred.user.avatar_url))
        try:
            del guild_dict[guild.id]['configure_dict']['settings']['config_sessions'][owner.id]
        except KeyError:
            pass
...
async def _configure_welcome(ctx):
    ...
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=welcomeconfig).set_author(name=_('Welcome Message'), icon_url=Alfred.user.avatar_url))
    while True:
        welcomereply = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
        if welcomereply.content.lower() == 'y':
            ...
            await owner.send(embed=discord.Embed(
                ...
                description=(_("Would you like a custom welcome message? "
                             ...
                             "Surround your message with [] to send it as an embed. **Warning:** Mentions within embeds may be broken on mobile, this is a Discord bug."))).set_author(name=_("Welcome Message"), icon_url=Alfred.user.avatar_url))
            if config_dict_temp['welcome']['welcomemsg'] != 'default':
                await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=config_dict_temp['welcome']['welcomemsg']).set_author(name=_("Current Welcome Message"), icon_url=Alfred.user.avatar_url))
            while True:
                welcomemsgreply = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and (message.author == owner)))
                if welcomemsgreply.content.lower() == 'n':
                    ...
                    if timeout or res.emoji == '❎':
                        ...
                        break
                break
            await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_("Which channel in your server would you like me to post the Welcome Messages? You can also choose to have them sent to the new member via Direct Message (DM) instead.\n\nRespond with: **channel-name** or ID of a channel in your server or **DM** to Direct Message:")).set_author(name=_("Welcome Message Channel"), icon_url=Alfred.user.avatar_url))
            while True:
                welcomechannelreply = await Alfred.wait_for('message',check=lambda message: message.guild == None and message.author == owner)
                if welcomechannelreply.content.lower() == "dm":
                    ...
                    if (not diff):
                        ...
                        ow = channel.overwrites_for(Alfred.user)
                        ow.send_messages = True
                        ow.read_messages = True
                        ow.manage_roles = True
                        try:
                            await channel.set_permissions(Alfred.user, overwrite = ow)
                        except (discord.errors.Forbidden, discord.errors.HTTPException, discord.errors.InvalidArgument):
...
async def _configure_exraid(ctx):
    ...
    if config_dict_temp['exraid']['enabled']:
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('For each report, I generate Google Maps links to give people directions to EX raids! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.')).set_author(name=_('EX Raid Reporting Locations'), icon_url=Alfred.user.avatar_url))
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('{citychannel_list}').format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name=_('Entered Channels'), icon_url=Alfred.user.avatar_url))
        while True:
            cities = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
            if cities.content.lower() == 'cancel':
        ...
        while True:
            ...
            if categories.content.lower() == 'cancel':
                ...
                while True:
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(),description=_("In the same order as they appear below, please give the names of the categories you would like raids reported in each channel to appear in. You do not need to use different categories for each channel, but they do need to be pre-existing categories. Separate each category name with a comma. Response can be either category name or ID.\n\nExample: `kansas city, hull, 1231231241561337813`\n\nYou have configured the following channels as EX raid reporting channels.")).set_author(name=_('EX Raid Reporting Categories'), icon_url=Alfred.user.avatar_url))
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('{citychannel_list}').format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name=_('Entered Channels'), icon_url=Alfred.user.avatar_url))
                    regioncats = await Alfred.wait_for('message', check=lambda message: message.guild == None and message.author == owner)
                    if regioncats.content.lower() == "cancel":
                        await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description=_('**CONFIG CANCELLED!**\n\nNo changes have been made.')))
        ...
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_("Who do you want to be able to **see** the EX Raid channels? Your options are:\n\n**everyone** - To have everyone be able to see all reported EX Raids\n**same** - To only allow those with access to the reporting channel.")).set_author(name=_('EX Raid Channel Read Permissions'), icon_url=Alfred.user.avatar_url))
        while True:
            permsconfigset = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
...
async def _configure_wild(ctx):
    ...
    if config_dict_temp['wild']['enabled']:
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('For each report, I generate Google Maps links to give people directions to wild spawns! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.')).set_author(name=_('Wild Reporting Locations'), icon_url=Alfred.user.avatar_url))
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('{citychannel_list}').format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name=_('Entered Channels'), icon_url=Alfred.user.avatar_url))
        while True:
            cities = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
            if cities.content.lower() == 'cancel':
...
async def _configure_research(ctx):
    ...
    if config_dict_temp['research']['enabled']:
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('For each report, I generate Google Maps links to give people directions to field research! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.')).set_author(name=_('Research Reporting Locations'), icon_url=Alfred.user.avatar_url))
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('{citychannel_list}').format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name=_('Entered Channels'), icon_url=Alfred.user.avatar_url))
        while True:
            cities = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
            if cities.content.lower() == 'cancel':
...
async def _configure_meetup(ctx):
    ...
    if config_dict_temp['meetup']['enabled']:
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('For each report, I generate Google Maps links to give people directions to meetups! To do this, I need to know which suburb/town/region each report channel represents, to ensure we get the right location in the map. For each report channel you provided, I will need its corresponding general location using only letters and spaces, with each location seperated by a comma and space.\n\nExample: `kansas city mo, hull uk, sydney nsw australia`\n\nEach location will have to be in the same order as you provided the channels in the previous question.\n\nRespond with: **location info, location info, location info** each matching the order of the previous channel list below.')).set_author(name=_('Meetup Reporting Locations'), icon_url=Alfred.user.avatar_url))
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('{citychannel_list}').format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name=_('Entered Channels'), icon_url=Alfred.user.avatar_url))
        while True:
            cities = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
            if cities.content.lower() == 'cancel':
        ...
        while True:
            ...
            if categories.content.lower() == 'cancel':
                ...
                while True:
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(),description=_("In the same order as they appear below, please give the names of the categories you would like raids reported in each channel to appear in. You do not need to use different categories for each channel, but they do need to be pre-existing categories. Separate each category name with a comma. Response can be either category name or ID.\n\nExample: `kansas city, hull, 1231231241561337813`\n\nYou have configured the following channels as meetup reporting channels.")).set_author(name=_('Meetup Reporting Categories'), icon_url=Alfred.user.avatar_url))
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_('{citychannel_list}').format(citychannel_list=citychannels.content.lower()[:2000])).set_author(name=_('Entered Channels'), icon_url=Alfred.user.avatar_url))
                    regioncats = await Alfred.wait_for('message', check=lambda message: message.guild == None and message.author == owner)
                    if regioncats.content.lower() == "cancel":
                        await owner.send(embed=discord.Embed(colour=discord.Colour.red(), description=_('**CONFIG CANCELLED!**\n\nNo changes have been made.')))
...
async def _configure_archive(ctx):
    ...
    if config_dict_temp['archive']['enabled']:
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_("I can also listen in your raid channels for words or phrases that you want to trigger an automatic archival. For example, if discussion of spoofing is against your server rules, you might tell me to listen for the word 'spoofing'.\n\nReply with **none** to disable this feature, or reply with a comma separated list of phrases you want me to listen in raid channels for.")).set_author(name=_('Archive Configuration'), icon_url=Alfred.user.avatar_url))
        phrasemsg = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
...
async def _configure_trade(ctx):
    ...
    owner = ctx.message.author
    config_dict_temp = getattr(ctx, 'config_dict_temp',copy.deepcopy(guild_dict[guild.id]['configure_dict']))
    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=_("The **!trade** command allows your users to organize and coordinate trades. This command requires at least one channel specifically for trades.\n\nIf you would like to disable this feature, reply with **N**. Otherwise, just send the names or IDs of the channels you want to allow the **!trade** command in, separated by commas.")).set_author(name=_('Trade Configuration'), icon_url=Alfred.user.avatar_url))
    while True:
        trademsg = await Alfred.wait_for('message', check=(lambda message: (message.guild == None) and message.author == owner))
...
async def changeraid(ctx, newraid):
    ...
    if newraid.isdigit():
        ...
        raid_img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/eggs/{}?cache=1'.format(str(egg_img))
        raid_message = await channel.get_message(guild_dict[guild.id]['raidchannel_dict'][channel.id]['raidmessage'])
        report_channel = Alfred.get_channel(raid_message.raw_channel_mentions[0])
        ...
        if changefrom == "egg":
            ...
            raid_message.content = re.sub(_('Alfred!\s.*\sraid\sreported'),_('Level {} reported').format(newraid), raid_message.content, flags=re.IGNORECASE)
            report_message.content = re.sub(_('Alfred!\s.*\sraid\sreported'),_('Level {}').format(newraid), report_message.content, flags=re.IGNORECASE)
...
@Alfred.command(name='uptime')
async def cmd_uptime(ctx):
    "Shows Alfred's uptime"
    guild = ctx.guild
    channel = ctx.channel
    embed_colour = guild.me.colour or discord.Colour.lighter_grey()
    uptime_str = await _uptime(Alfred)
    embed = discord.Embed(colour=embed_colour, icon_url=Alfred.user.avatar_url)
...
async def about(ctx):
    ...
    embed = discord.Embed(colour=embed_colour, icon_url=Alfred.user.avatar_url)
    embed.add_field(name=_('About Alfred'), value=about, inline=False)
...
async def leaderboard(ctx, type="total"):
    ...
    embed = discord.Embed(colour=ctx.guild.me.colour)
    embed.set_author(name=_("Reporting Leaderboard ({type})").format(type=type.title()), icon_url=Alfred.user.avatar_url)
...
async def want(ctx,*,pokemon):
    ...
    Usage: !want <species>
    Alfred will mention you if anyone reports seeing
    this species in their !wild or !raid command."""

    """Behind the scenes, Alfred tracks user !wants by
    ...
    if (len(want_list) == 1) and ((len(added_list) == 1) or (len(spellcheck_dict) == 1) or (len(already_want_list) == 1)):
        if len(added_list) == 1:
            ...
            want_img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/pkmn/{0}_.png?cache=2'.format(str(want_number).zfill(3))
...
async def unwant(ctx,*,pokemon):
    ...
    """Behind the scenes, Alfred removes the user from
...
async def _wild(message, content):
    ...
    if wild_number == 25:
        wild_img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/pkmn/{0}witch_.png?cache=2'.format(str(wild_number).zfill(3))
    else:
        wild_img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/pkmn/{0}_.png?cache=2'.format(str(wild_number).zfill(3))
...
@Alfred.command(aliases=['r', 're', 'egg', 'regg', 'raidegg'])
...
async def raid(ctx,pokemon,*,location:commands.clean_content(fix_channel_mentions=True)="", weather=None, timer=None):
    ...
    Alfred will insert <location> into a
    Google maps link and post the link to the same channel the report was made in.
    Alfred's message will also include the type weaknesses of the boss.

    Finally, Alfred will create a separate channel for the raid report, for the purposes of organizing the raid."""
    content = f"{pokemon} {location}"
    if pokemon.isdigit():
...
async def _raid(message, content):
    ...
    if raidexp is not False:
        if _timercheck(raidexp, raid_info['raid_eggs'][get_level(entered_raid)]['raidtime']):
            await message.channel.send(_("Alfred...that's too long. Level {raidlevel} raids currently last no more than {raidtime} minutes...").format(raidlevel=get_level(entered_raid), raidtime=raid_info['raid_eggs'][get_level(entered_raid)]['raidtime']))
            return
    ...
    raid_img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/pkmn/{0}_.png?cache=2'.format(str(raid_number).zfill(3))
...
async def _raidegg(message, content):
    ...
    if raidexp is not False:
        if _timercheck(raidexp, raid_info['raid_eggs'][str(egg_level)]['hatchtime']):
            await message.channel.send(_("Alfred...that's too long. Level {raidlevel} Raid Eggs currently last no more than {hatchtime} minutes...").format(raidlevel=egg_level, hatchtime=raid_info['raid_eggs'][str(egg_level)]['hatchtime']))
            return
    ...
    if (egg_level > 5) or (egg_level == 0):
        ...
        try:
            ...
            pass
        raid_img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/eggs/{}?cache=2'.format(str(egg_img))
...
async def _eggassume(args, raid_channel, author=None):
    ...
    report_channel = Alfred.get_channel(eggdetails['reportcity'])
    egglevel = eggdetails['egglevel']
...
async def _eggtoraid(entered_raid, raid_channel, author=None):
    ...
    try:
        reportcitychannel = Alfred.get_channel(eggdetails['reportcity'])
...
async def research(ctx, *, details = None):
    ...
    research_embed = discord.Embed(colour=message.guild.me.colour).set_thumbnail(url='https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/misc/field-research.png?cache=2')
    ...
    while True:
        if details:
            ...
            try:
                pokestopmsg = await Alfred.wait_for('message', timeout=60, check=(lambda reply: reply.author == message.author))
            except asyncio.TimeoutError:
                pokestopmsg = None
            ...
            try:
                questmsg = await Alfred.wait_for('message', timeout=60, check=(lambda reply: reply.author == message.author))
            except asyncio.TimeoutError:
                questmsg = None
            ...
            try:
                rewardmsg = await Alfred.wait_for('message', timeout=60, check=(lambda reply: reply.author == message.author))
            except asyncio.TimeoutError:
                rewardmsg = None
...
async def meetup(ctx, *,location:commands.clean_content(fix_channel_mentions=True)=""):
    ...
    Finally, Alfred will create a separate channel for the report, for the purposes of organizing the event."""
...
async def _meetup(ctx, location):
    ...
    try:
        ...
        pass
    raid_img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/misc/meetup.png?cache=2'
...
async def timerset(ctx, *,timer):
    ...
    Alfred displays the end time in HH:MM local time."""
    message = ctx.message
    channel = message.channel
    guild = message.guild
    hourminute = False
    ...
    if (not checks.check_exraidchannel(ctx)) and not (checks.check_meetupchannel(ctx)):
        ...
        if _timercheck(raidexp, maxtime):
            await channel.send(_("Alfred...that's too long. Level {raidlevel} {raidtype}s currently last no more than {maxtime} minutes...").format(raidlevel=str(raidlevel), raidtype=raidtype.capitalize(), maxtime=str(maxtime)))
            return
...
async def _timerset(raidchannel, exptime):
    ...
    report_channel = Alfred.get_channel(guild_dict[guild.id]['raidchannel_dict'][raidchannel.id]['reportcity'])
    ...
    try:
        ...
        pass
    raidchannel = Alfred.get_channel(raidchannel.id)
    event_loop.create_task(expiry_check(raidchannel))

@Alfred.command()
@checks.raidchannel()
async def timer(ctx):
    """Have Alfred resend the expire time message for a raid.
...
async def starttime(ctx,*,start_time=""):
    ...
    if (start and now <= start) or timeset:
        ...
        report_channel = Alfred.get_channel(rc_d['reportcity'])
...
@Alfred.group(case_insensitive=True)
@checks.activechannel()
async def location(ctx):
    ...
    if ctx.invoked_subcommand == None:
        ...
        report_channel = Alfred.get_channel(rc_d[channel.id]['reportcity'])
...
async def duplicate(ctx):
    ...
    Alfred deactivates the channel and marks it for deletion."""
    channel = ctx.channel
    author = ctx.author
    guild = ctx.guild
    rc_d = guild_dict[guild.id]['raidchannel_dict'][channel.id]
...
async def counters(ctx, *, args = None):
    ...
    If [user] is a valid Pokebattler user id, Alfred will simulate the Raid with that user's Pokebox.
...
async def _counters(ctx, pkmn, user = None, weather = None, movesetstr = "Unknown Moveset"):
    img_url = 'https://raw.githubusercontent.com/FoglyOgly/Alfred/discordpy-v1/images/pkmn/{0}_.png?cache=4'.format(str(get_number(pkmn)).zfill(3))
...
@Alfred.command(aliases=['i', 'maybe'])
@checks.activechannel()
...
async def _edit_party(channel, author=None):
    ...
    reportchannel = Alfred.get_channel(guild_dict[channel.guild.id]['raidchannel_dict'][channel.id]['reportcity'])
    try:
...
async def starting(ctx, team: str = ''):
    ...
    if starttime:
        ...
        report_channel = Alfred.get_channel(guild_dict[ctx.guild.id]['raidchannel_dict'][ctx.channel.id]['reportcity'])
...
@Alfred.group(name="list", aliases=['lists'], case_insensitive=True)
async def _list(ctx):
    ...
    if ctx.invoked_subcommand == None:
        ...
        if checks.check_raidreport(ctx) or checks.check_exraidreport(ctx):
            ...
            event_list = []
            for r in rc_d:
                reportcity = Alfred.get_channel(rc_d[r]['reportcity'])
                if not reportcity:
                    continue
...
try:
    event_loop.run_until_complete(Alfred.start(config['bot_token']))
except discord.LoginFailure:
    logger.critical('Invalid token')
    event_loop.run_until_complete(Alfred.logout())
    Alfred._shutdown_mode = 0
    ...
    pass
sys.exit(Alfred._shutdown_mode)