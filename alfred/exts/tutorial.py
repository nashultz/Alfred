...
class Tutorial:
    ...
    async def raid_tutorial(self, ctx, config):
        ...
        await asyncio.sleep(1)
        await raid_channel.send(
            "A couple of notes about raid channels. Alfred has "
    ...
    async def tutorial(self, ctx):
        """Launches an interactive tutorial session for Alfred.

        Alfred will create a private channel and initiate a
        ...
        await ctx.tutorial_channel.send(
            f"Hi {ctx.author.mention}! I'm Alfred, a Discord helper bot for "
        ...
        try:
            ...
            # finish tutorial
            await ctx.tutorial_channel.send(
                f"This concludes the Alfred tutorial! "
